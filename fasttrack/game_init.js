...
window.GameInit = (function () {
    ...
    function parseURLParams() {
        ...
        // Auto-start signals from the unified /play/ launcher.
        // ANY of these means "skip the embedded wizard, boot straight into a game":
        //   launch=1       — standard /play/ flag
        //   autostart=1    — alias used for some games
        //   offline=true   — /play/ adds this for FastTrack solo
        //   quickplay=1    — legacy in-game shortcut (kept for back-compat)
        const auto = params.get('launch') === '1'
            || params.get('autostart') === '1'
            || params.get('offline') === 'true'
            || params.get('quickplay') === '1';

        // Player count: prefer /play/'s ai_players (humans always = 1 in solo)
        // then fall back to legacy `players` (total seats including human).
        ...
        if (aiPlayersRaw !== null) {
            ...
            totalPlayers = clamp((isFinite(ai) ? ai : 1) + 1, 2, 4);
        } else if (playersRaw !== null) {
            const n = parseInt(playersRaw, 10);
            totalPlayers = clamp(isFinite(n) ? n : DEFAULT_CONFIG.defaultPlayers, 2, 4);
        } else {
            totalPlayers = DEFAULT_CONFIG.defaultPlayers;
        }

        // Difficulty: /play/ uses ai_level (easy|medium|hard); legacy used difficulty.
        // FastTrack engine accepts: easy | normal | hard.
        const aiLevel = params.get('ai_level');
        const diffRaw = aiLevel || params.get('difficulty') || DEFAULT_CONFIG.defaultDifficulty;
        const difficulty = (diffRaw === 'medium') ? 'normal' : diffRaw;
